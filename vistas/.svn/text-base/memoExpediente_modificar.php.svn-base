...
if ($indReg != '1' and $indReg != '2' and $indReg != '3' and $indReg != '5' and $id ) {

	$memo = $objBeneficiario->listarMemo($id);

	$data = $objBeneficiario->listarDetMemo($id);
	...
	$_SESSION['detalleMemoE'] = NULL;
		...
		for ($i = 0; $i < count($data); $i++) {
				$_SESSION['detalleMemoE'][] = $data[$i];
			}

	}
}
//Ojo $ban es una bandera
...
if ($indReg == '1') {
	...
	if (!$busqueda) {
		...
		echo '<script>alert("Este Numero ' . $rif . ' ya esta ingresado")</script>';
	//    else
...
if ($indReg == '3') {
	...
	$regrif = $objBeneficiario->registrarMemorif($nummemo, $_SESSION['banco'], $_SESSION['atencion'], $nota, $_SESSION['detalleMemoE'],$id);
...
<form action="" method="post" name="registro">
 ...
 <input type="hidden" name="bancoid" id="bancoid" value="<?php if ($ban) echo $memo[4]; else echo $_SESSION['banco']; ?>">
 <fieldset class="form">
  <legend>Datos del Memo Expediente <?php if ($ban) echo str_pad($id,6,'0',STR_PAD_LEFT)?>
  </legend>
     <table  align="center" >
        <!--  <tr>
		  ...
		  <?php } else { echo $listarBancos2[1]; ?>
		  	 <input type="hidden" name="banco" id="banco" value="<?php echo $memo[4]; ?>">
		  	 <?php } ?>
          </td>
		   </tr>
		   <tr>
           ...
           <td align="left">
             <input name="atencion" type="text" id="atencion" value="<?php if ($ban) echo $memo[6]; else echo $listarBancos2[2];?>" />
           </td>
          </tr>
          <tr>
           <td  class="categoria">Nota:</td>
           <td align="left">
            <textarea name="nota" cols="100" rows="3" id="nota" onblur="javascript:this.value=this.value.toUpperCase()" ><?php  if ($ban) echo $memo[5]; echo $_POST['nota'];?></textarea>
             </td>
          </tr>
		   <tr>
...
$indC = false;
for ($i = 0; $i < count($_SESSION['detalleMemoE']); $i += 42) {
	if (!$indC) {
		$color = 'datospar';
		$indC = true;
	} else {
...
?>
	    ...
	    <TD ><div align="center"><a class="vinculo" href="javascript: eliminar('5','<?php  echo $i/41;?>')"><img src="botones/delete.png" width="29" height="29" border="0"></a></div></TD>
	    </TR>
        <?php  }   ?>
	  </TABLE>
    </TD>
