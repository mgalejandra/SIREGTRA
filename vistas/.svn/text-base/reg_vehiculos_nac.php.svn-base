...
$dir='http://'.$host.$uri;
$permitidos = array(1,2,3,4,5,11,18,23,24);
...
if ($idsercarveh and $indReg!=2)  {
	...
	$i=0;
	//echo 'entro';
	$listVehic=$objVehiculo->listarVehiculos($idsercarveh,'','','','N','','');
}
...
<!--  Contenido Principal         -->
  ...
  <table class="formulario" width="822" border="0" align="center" >
      <tr>
        <td colspan="4" class="cabecera">Registrar Datos de Vehiculos  Nacionales / Exportados y Programa Venezuela Móvil </td>
      </tr>
      <tr>
      ...
      <input type="hidden" name="indReg" >
       <tr>
        <td class="categoria">Id&nbsp;Característica&nbsp;Vehículo</td>
        <td class="dato" colspan='4' >
         <input type="button" onclick="catalogoAncho('cat_caracte_nac.php');" value="..." />
         <input name="desidCaract" type="text" id="desidCaract"   value="<?php if($ban==1)  echo $listVehic[$i+7];?>"  size="67"  readonly=""/>
         </td>
      </tr>
       <tr>
		...
		<td><input name="idCaract" type="hidden" id="idCaract" value="<?php if($ban==1)  echo $listVehic[$i+8];?>" size="20"  maxlength="15" onblur="javascript:this.value=this.value.toUpperCase()" /></td>
       </tr>
      <tr>
        <td height="22" colspan="4">
          <div align="center">
           <?php if (!$idsercarveh) { ?>
            ...
            <?php } if ($idsercarveh and ($_SESSION['tipoUsuario'] == 1 or $_SESSION['tipoUsuario'] == 4)) { ?>
            <input name="Modificar" type="button" id="Modificar" onclick="validarCaract('2'); return false" value="Modificar" />
            <?php } ?>
            <input name="listar" type="button" id="listar" onclick="window.location.href='listado_vehiculos_nac.php'" value="Listar" />
         </div>
     </tr>
 </table>
    </form>
