...
//echo "EL concesionario es: ".$_SESSION['id_concesionario'];
...
$dir='http://'.$host.$uri;
$permitidos = array(1,2,3,4,11,13,14,15,17,19);
validaAcceso($permitidos,$dir);
$ban=0;

$listarTipoRec=$objReclamo->listarTipoR('',2);
...
if ($idreclamo and $indReg!=2)  {
	...
	$i=0;
	//echo 'entro';
	$listarTipoR=$objConcesionario->listarReclamosConc($_SESSION['id_concesionario'],'','',-1,$idreclamo);
}

if ($indReg==1){

    $datos = array($ced,$nom1,$nom2,$ape1,$ape2,$idtipo,$sercarveh,$descr,$resp,$tlf1,$tlf2,$banco,$nomcom,$sexo,$_SESSION['id_concesionario']);

   if($indErr)echo '<SCRIPT>alert("'.$msj.'")</SCRIPT>';
   else  $registro = $objConcesionario->registrarReclamoC($datos);

	if ($registro)  {
		 echo "<script>alert('Reclamo Registrado');</script>";
		 echo "<SCRIPT>window.location.href='listado_reclamos_concesionario.php';</SCRIPT>";
		  $_SESSION['tipo']=NULL;
		  $_SESSION['ced']=null;
	}
}
...
if ($indReg==2){
	...
	$datos = array($ced,$nom1,$nom2,$ape1,$ape2,$_SESSION['tipo'],$sercarveh,$descr,$resp,$tlf1,$tlf2,$banco,$nomcom,$sexo,$_SESSION['id_concesionario']);
...
//	echo "Cedula: ".$_SESSION['ced'];
	 ...
	 if ($datoBen){
		  ...
		  $buscarSerial=$objAsignacion->listarAsignacion1('',$_SESSION['ced'],'','','','',-1);
		  $serial=$buscarSerial[0];
	 }
 ...
 if(!$_SESSION['tipo']){
   ...
   elseif($_SESSION['tipo'] == 'otro_tipo'){
       ...
       elseif(!validarString($nuevoTipo)){
          ...
          $indTipo = $objReclamo->registrarTipoR($nuevoTipo,2);
          if($indTipo){
             $_SESSION['tipo'] = count($objReclamo->listarTipoR('',2))/$nroR;
            }
          else{
             $indErr == true;
...
<!--  Contenido Principal         -->
  ...
  <table class="formulario" width="822" border="0" align="center" >
      ...
      <tr>
        ...
        <td class="dato" >
          <input name="sercarveh" type="text" id="sercarveh" onblur="javascript:this.value=this.value.toUpperCase()" value="<?php if($ban==1)  echo $listarTipoR[8];elseif ($ban==2) echo $serial;?>" size="25" maxlength="18"/>
       </td>
        ...
        <td height="22" colspan="4">
          <div align="center">
           ...
           <?php if ($ban<>1) { ?>
            ...
            <?php } ?>
            <input name="listar" type="button" id="listar" onclick="window.location.href='listado_reclamos_concesionario.php'" value="Listar" />
         </div>
     </tr>
 </table>
    </form>
