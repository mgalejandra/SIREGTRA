...
$cantReg=$objVines->listadodevinesmovi($tipo,$modelo,$marca,$color,$pdi,$fecha,$Hora,$nro);

$cantSer=$objVines->contarlistadodevinesmovi($tipo,$modelo,$marca,$color,$pdi,$fecha,$Hora,$nro);
...
$listadodevines=$objVines->listadodevinesmovi($tipo,$modelo,$marca,$color,$pdi,$fecha,$Hora,$nro,$offset);
...
function exel(URL) {
	  ...
	  id = day.getTime();
	  eval("page" + id +
	  	   " = window.open('reportes/xlslistadovinesmovi.php?tipo=<?php echo $tipo;?>&modelo=<?php echo $modelo;?>&marca=<?php echo $marca;?>&color=<?php echo $color;?>&pdi=<?php echo $pdi;?>&fecha=<?php echo $fecha;?>&hora=<?php echo $hora;?>&nro=<?php echo $nro;?>&id='+URL,'','toolbar=0,scrollbars=1,location=0,statusbar=1,menubar=0,resizable=0,width=1100,height=900');");
...
<fieldset class="form">
  <legend>Listado de Movimientos Cantidad de Vehiculos: <?php echo $cantSer[0]; ?></legend>
  <TABLE class="dato" border="0" width="900" align="center">
 <tr>
...
<!-- 			  			<a class="vinculo" target="_blank" onClick="abrir('reportes/pdfCuadroResCitasxBanco.php?tipo=<? echo$tipoBen;?>&fechaD=<? echo$fechaD;?>&fechaH=<? echo$fechaH; ?>&banco=<? echo$banco;?>');" />
		   ...
		   <TH colspan="8"><? echo "Resumen de PDI"?></TH>
		   <TH colspan="3"><? echo "Fechas"?></TH>
     	   </TR>
		   <TR class="cabeceraI">
		   <TH >Nro</TH>
		    <TH >Tipo</TH>
...
<?php
	      ...
	      for($i=0;$i<count($listadodevines);$i+=23)
	      {
              ...
              if ($ref<>$listadodevines[$i+5] and !$pri){
              	    echo "<td class='cabeceraI' colspan='20'></td>";
              }

              $ref=$listadodevines[$i+5];
               $pri=false;
     ?>
