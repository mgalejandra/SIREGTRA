...
require('../modelos/factura.php');
	...
	$dir='http://'.$host.$uri;
	$permitidos = array(1,2,3,4,5,6,7,8,11,15,17,18);
...
if($indReg=='1'){
$busqueda=false;
                   $listarBeneficiario=$objBeneficiario->listarBeneficiario($rif,'','','','','1');
                   $buscarsiyaesta=$objBeneficiario->listarEstatusCredito('','',$rif,'','',-1,null,'1');
                   $buscarentregados=$objFactura->listarFacturas('','','','','-1',$rif,'','','','','','','','','','','' ,'','','','','','','','','','','','','','','','','','1','','','');
                   if($buscarentregados[10]=='15') $entregado=true;
                  //  echo 'aqui'.count($buscarentregados);
                  // echo 'estatus:'.$buscarentregados[10].' cedula '.$buscarentregados[8];
             ...
             //      if($_SESSION['banco']==$listarBeneficiario[40])
                       $bus=false;
	                   if (!$busqueda and count($buscarsiyaesta)==0 and !$entregado){
			              ...
			              $bus=true;
	                   	...
	                   	echo '<script>alert("Esta Persona '.$rif.' ya esta tiene un vehiculo entregado")</script>';
	                   }
		       //    else
...
$regrif=$objBeneficiario->registroEstatusCredito($_SESSION['banco'],$_SESSION['detalleMemoE'],$_SESSION['estatus']);
...
//echo 'aqui: '.count($regrif);
					 ...
					 echo "<SCRIPT>window.location.href='listado_estatusCredito.php';</SCRIPT>";
...
function enviar(dato){
	    ...
	    monto=document.registro.monto.value;
