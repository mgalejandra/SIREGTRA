...
if($idMod == 1){
   $ced = $objUsuario->buscarUsuario($valor);
   $cambioClave = $objUsuario->cambiarClave($valor,$ced[5]);
...
$contArt = $objUsuario->contarUsuario($usuario,$cedula,$tipo,$_SESSION['nivelUsu'],$nombre,$estatus,$usuario2,$banco,$dep);
...
$resUsuario = $objUsuario->buscarUsuario($usuario,$cedula,$tipo,$_SESSION['nivelUsu'],$offset,$nombre,$estatus,$usuario2,$banco,$dep);
...
if ($limp){
	...
	$listarDep=$objDep->listarDepartamento($numdep,$descdep);
?>
<!DOCTYPE HTML PUBLIC >
<html>
  <head>
  ...
  <legend>Criterios de Busqueda
     ...
     <table  align="center" >
      <tr>
        ...
        <TD class="dato"><INPUT type="text" name="usuario2" ></TD>
      ...
      <tr>
	    ...
	    <TD class="dato">
		       <select name="tipo" >
		       <option value=""></option>
			         <?php   $tipoUsuario = $objUsuario->retTipo();
...
<?php for($i=0;$i<count($resUsuario);$i+=$nroCampos){
 ...
 <TR class="<?php echo $color ?>">
   ...
   <TD><?php echo $resUsuario[$i+14];   ?></TD>
   ...
   <?php if($resUsuario[$i+7] != 'BLOQUEADO'){?>
    ...
    <td><div align="center">
     <a class="vinculo" href="mod_usuario.php?id=<?=$resUsuario[$i]?>&indReg=1"> <img src="imagenes/edit_f2.png" width="30" height="30">
	  </a></div></td>
 </TR>
<?php }?>
