...
$dir='http://'.$host.$uri;
$permitidos = array(1,2,3,4,5,6,7,13,17);
...
if ($_GET['idpago']) $id_pago = $_GET['idpago']; else $listarPago=null;
  ...
  if ($num){
  	  ...
  	  //echo $listarFactura.'aqui prueba';
  }else
  {
...
if ($id_pago or $indReg!=2)  {
	...
	$i=0;
	//echo 'entro';
	$listarPago = $objPago->buscarPago_Anulado($id_pago);
...
*/
	if($indReg==2){
	  ...
	  $i=0;
		...
		if (($sercarveh!=$listarPago[+7]) and ($codproc==$listarPago[+12])){
			$modificar = $objPago->ActivarPagoAnulado($id_pago,$datos,$statusPago,$codproc,$num);

			if ($modificar){
				//echo "Debo cambiar ahora el estatus de la proforma";
				$modificarEstatus = $objPago->modificarStatusalCambiarPago($datos[5],$datos[6]);

				if ($modificarEstatus){
					f_alert("El Pago ha sido Activado, y la proforma actualizada");
					echo "<SCRIPT>window.location.href='listado_pagos.php?id_caso=2';</SCRIPT>";
				}
			}
			else{
				 f_alert("No se pudo Activar el pago verifique por favor si ya el beneficiario tiene una factura proforma asociada");
			 	 echo "<SCRIPT>window.location.href='listado_factura.php';</SCRIPT>";
			}
		}
		else
			f_alert("Debe seleccionar el NUEVO serial del vehículo asignado al Beneficiario con CI:".$listarPago[+12]);
	}
//}
  ...
  <table class="formulario" width="900" border="0" align="center" >
      ...
      <tr>
        ...
        <td class="dato" colspan="2">
	       ...
	       <!-- <input type="hidden" name="idAsig" id="idAsig"
	        	...
	        	<input type="text" name="codproa" id="codproa" value="<?php if ($num) echo $listarFactura[13];?><?=($ban==1)?$listarPago[$i+12]:$codproa?>"size="40" maxlength="20" readonly="" />
        </td>
      </tr>
    ...
    <tr>
        <td class="categoria" colspan="1"> Banco:&nbsp;</td>
			 ...
			 <SELECT id="id_banco" name="id_banco" onchange="fcodBanco()" onkeydown="fcodBanco()" onkeyup="fcodBanco()" readonly="" >
				<OPTION value=""></OPTION>
      ...
      <td class="dato" colspan="2" title="Número cuenta del banco origen del Pago">
	       ...
	       <input name="numCtaBanco" type ="text" id="numCtaBanco"
	       	value="<?=($ban==1)?$listarPago[$i+9]:$numCtaBanco?>" size="16" maxlength="16" width="16" readonly=""
...
<tr>
      ...
      <td class="dato" colspan="1">
	       ...
	       value="<?=($ban==1)?$listarPago[$i+1]:$numPago?>" size="10" maxlength="10" readonly=""
	       onkeypress="return acessoNumerico(event)"/>
      </td>
      ...
      <!--    <img src="../images/cal.gif" width="16" height="18"
          		onClick="show_calendar('document.forms[0].fecPago',document.forms[0].fecPago.value)" readonly="" /> -->
        </td>
</tr>
<tr>
        ...
        <td class="dato" colspan="2">
	       <input name="monto" type ="text" id="monto" onkeypress="return acessoDecimal(event)"
	       		  ...
	       		  maxlength="10"   onblur="javascript:this.value=this.value.toUpperCase()"  readonly="" />
       </td>
       <td class="categoria"colspan="1">Tipo&nbsp;Pago:&nbsp;</td>
        <td class="dato" colspan="2">
        <input  name="tipo" id="tipo"
            value="<?php echo $listarPago[$i+11]?>" readonly=""/>
          </td>
          </td>
</tr>
<tr>
    <td height="22" colspan="8">
       <div align="center">
          <input type="hidden" name="indReg" >
           ...
           <?php } if ($id_pago and ($_SESSION['tipoUsuario'] == 1 or $_SESSION['tipoUsuario'] == 4 or $_SESSION['tipoUsuario'] == 17 )) { ?>

             <?php } echo $id_pago.'aqui'.$_SESSION['tipoUsuario'];?>
                 <input name="Modificar" type="button" id="Modificar" onclick="validarCaract('2'); return false" value="Activar Pago" />
           <input name="listar" type="button" id="listar" onclick="window.location.href='listado_pagos_anulados.php?id_caso=2'" value="Listar" />
       </div>
    </td>
</tr>
</table>
