...
$objenvios = new envios();
 ...
 $Misiont=$_GET['Misiont'];
 $fechaenvio=date("d/m/Y");

if ($origen == 'I')
{
	   if (!$tipo)
 		    $listarvehiculos=$objenvios->txtVehImp($numenv,$precioj,$Misiont);
 		if ($tipo)
 		    $listarvehiculos=$objenvios->txtVehImpElim($numenv,$precioj,$Misiont);
}
else
...
if ($origen=='I'){
    ...
    str_pad($listarvehiculos[$i+34],8,'0',STR_PAD_RIGHT).str_pad($listarvehiculos[$i+35],3,' ',STR_PAD_RIGHT).
    ...
    str_pad($listarvehiculos[$i+41],3,'0',STR_PAD_LEFT).
    str_pad('P',65,' ',STR_PAD_LEFT).chr(0x0D).chr(0x0A);
    
    
      }
      
      if    	($listarvehiculos[$i+6]<='2013') {
