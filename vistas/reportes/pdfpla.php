...
//$_SESSION['numenv']=7;
 ...
 $precioj= $_SESSION['precioj'];
  $rifcons="G200079843";
 	...
 	if ($gen == 'N')
        $listarvehiculos=$objenvios->listarPlaTxt($numlotveh,$tipo,'','',$precioj);
    else
        $listarvehiculos=$objenvios->listarPlaTxt('',$tipo,$numenv,'',$precioj);

      $pdf->ln();
      $pdf->SetXY(10,30);
      $pdf->SetFont("Arial","",12);
      $pdf->Cell(0,6,'Archivo Txt para Placas ',0,0,"C");
      $pdf->ln();
      ...
      $mm=0;
      $me=0;
      for($i=0;$i<count($listarvehiculos);$i+=26){
      $con++;
      $pdf->setjump(1.5);
      ...
      $pdf->SetFont('Arial','',5.5);
        ...
        if ($gen == 'S')
		 	{
		     $data=array((substr($rifcons,0,1))
						 ,(substr($rifcons,1,8))
						 ,(substr($rifcons,9,9))
						 ,($listarvehiculos[$i+19])
				 ...
				 $objenvios->registrarEnviostxtPla($data);
		 	}
    ...
    //  $pdf->Line(10,$pdf->GetY()+5,205,$pdf->GetY()+5);//LINEA HORIZONTAL
      ...
      if ($gen=='S'){
       ...
       $pdf->PutLink(trim($dir).'?numlotveh='.$_POST["numlotveh"].'&numenv='.$_POST["numenv"].'&filtro='.$_POST["filtro"].'&fecdes='.$_POST["fecdes"].'&fechas='.$_POST["fechas"].'&precioj='.$precioj,'Descargar TXT');
      }
