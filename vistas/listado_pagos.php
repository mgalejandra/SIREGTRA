...
require('../modelos/pago.php');
	...
	$dir='http://'.$host.$uri;
    $permitidos = array(1,2,3,4,5,6,7,8,11,13,17,18,22,23);
...
$contPago = $objPago->contarPagos($pago,$id_banco,$fec1 ,$fec2 ,$sercarveh,$codpro,$nombre,$id_caso,$codStatus,$tipo,$idpago);
...
//$listarPago=$objPago->listarPagos($pago,$id_banco,$fec1,$fec2,$sercarveh,$codpro,$nombre,$codStatus,$offset,$nroFilas,$id_caso,$tipo);
...
if($_POST['indBusq']==3){
$listarPago=$objPago->listarPagos($pago,$id_banco,$fec1,$fec2,$sercarveh,$codpro,$nombre,$codStatus,$offset,$nroFilas,$id_caso,$tipo);
	echo '<SCRIPT> window.open("../vistas/bitacoraFactura.php?id='.$listarPago[10].' " , "ventana1" , "width=600,height=600,scrollbars=NO") </SCRIPT>';
	$ctrl=$objPago->AnularPago($id_pago,$msj);
}

$contPago = $objPago->contarPagos($pago,$id_banco,$fec1 ,$fec2 ,$sercarveh,$codpro,$nombre,$id_caso,$codStatus,$tipo,$idpago);
$listarPago=$objPago->listarPagos($pago,$id_banco,$fec1,$fec2,$sercarveh,$codpro,$nombre,$codStatus,$offset,$nroFilas,$id_caso,$tipo,$idpago);
}
...
function enviar(dato){
	if(dato==2){
		 ...
		 document.registro.idpago.value 	= null;
...
function exel(URL) {
	  ...
	  id = day.getTime();
	  eval("page" + id +
	  	   " = window.open('reportes/xlsListadoPagos.php?num_pag=<?php echo $pago;?>&banco=<?php echo $id_banco;?>&fecP=<?php echo $fec1;?>&fec2P=<?php echo $fec2;?>&serial=<?php echo $sercarveh;?>&codpro=<?php echo $codpro;?>&nombre=<?php echo $nombre;?>&status=<?php echo $codStatus;?>&fila=<?php echo $nroFilas;?>&caso=<?php echo $id_caso;?>&tipo=<?php echo $tipo;?>&id='+URL,'','toolbar=0,scrollbars=1,location=0,statusbar=1,menubar=0,resizable=0,width=1100,height=900');");
}
	      function limpiar(){
	      //	 alert('entroo en javascript');
  document.registro.limp.value = 1;
}
//////////////////////////////////////////////////////////////////////////////////////////////////
...
<!--  Contenido Principal         -->
  ...
  <legend>Criterios de Busqueda
     ...
     <table  align="center" border="0">
        ...
        <tr>
          ...
          <td></td>
          <td class="categoria">Nro&nbsp;Pago:</td>
           ...
           <td  class="categoria">Status&nbsp;pago:</td>
			 ...
			 <SELECT id="codStatus" name="codStatus" onchange="fcodStatus()" onkeydown="fcodStatus()" onkeyup="fcodStatus()">
				<OPTION value=""></OPTION>
				<OPTION value="E" <?if($codStatus=="E")echo"selected='true'"?>>EFECTIVO</OPTION>
				<OPTION value="V" <?if($codStatus=="V")echo"selected='true'"?>>DEVUELTO</OPTION>
				<OPTION value="A" <?if($codStatus=="A")echo"selected='true'"?>>ANULADO</OPTION>
				<OPTION value="D" <?if($codStatus=="D")echo"selected='true'"?>>DEPOSITADO</OPTION>
			 </SELECT>
			 </td>
 ...
 <td class="categoria">CI/RIF:</td>
			...
			<input name="codpro" type="text" id="codpro" value="<?=$codpro?>" onblur="javascript:this.value=this.value.toUpperCase()" size="15" maxlength="18"/>
		  	        	 ...
		  	        	 <td class="categoria">ID_Pago:</td>
           <td align="left" title="Número de cheque / transferencia">
			<input name="idpago" type="text" id="idpago" value="<?=$idpago?>" onkeypress="return acessoNumerico(event)" size="10"/>
		  </td>

		    <td  class="categoria">Tipo:</td>
	        <td class="dato">Activos
...
<?if($id_caso==2)echo '<td class="cabecera" colspan="3" rowspan="2"></td>'?>
            </tr>
            <tr>
              <td class="cabecera">Nro</td>
